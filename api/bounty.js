...
async function uploadToOSS(buffer, mimeType = 'image/jpeg') {
  ...
  const key = `bounties/${Date.now()}-${Math.random().toString(16).slice(2)}.${ext}`;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.error('[api/bounty] database connect failed', error);
    try {
      await dbClient.end();
    } catch (_) {}
  ...
  try {
    if (req.method === 'GET') {
      const { rows } = await dbClient.query(
        `SELECT id, description, reward, image_url, created_at
         FROM bounties
    ...
    if (req.method === 'POST') {
      const { description, reward, imageBase64 } = req.body || {};
      ...
      const { rows } = await dbClient.query(
        `INSERT INTO bounties (description, reward, image_url)
         VALUES ($1, $2, $3)
         RETURNING id, description, reward, image_url, created_at`,
        [String(description).trim(), rewardAmount, imageUrl]
      );
    ...
    console.error('[api/bounty] handler failed', error);
    try {
      await dbClient.end();
    } catch (_) {}
